...
dist = InVar(
    "Uniform", ndraws=n, dist=uniform, distkwargs={"loc": 1, "scale": 4}, samplemethod=method
)
...
dist = InVar("Normal", ndraws=n, dist=norm, distkwargs={"loc": 5, "scale": 2}, samplemethod=method)
...
dist = InVar(
    ...
    ndraws=n,
    dist=lognorm,
    distkwargs={"s": 0.5, "scale": np.exp(1)},
    samplemethod=method,
)
...
dist = InVar("Exponential", ndraws=n, dist=expon, distkwargs={"scale": 1 / 1}, samplemethod=method)
...
dist = InVar(
    "Random Integers with Custom Weights",
    ndraws=n,
    dist=custom_dist,
    distkwargs=dict(),
    samplemethod=method,
...
dist = InVar("Bernoulli", ndraws=n, dist=bernoulli, distkwargs={"p": 0.2}, samplemethod=method)
fig, _ = dist.plot(title="dist=bernoulli, distkwargs={'p': 0.2}")
fig.set_size_inches(6.0, 2.5)
plt.savefig("docs/images/bernoulli.png", dpi=100)

dist = InVar("Binomial", ndraws=n, dist=binom, distkwargs={"n": 10, "p": 0.2}, samplemethod=method)
fig, _ = dist.plot(title="dist=binom, distkwargs={'n': 10, 'p': 0.2}")
fig.set_size_inches(6.0, 2.5)
plt.savefig("docs/images/binom.png", dpi=100)

dist = InVar("Geometric", ndraws=n, dist=geom, distkwargs={"p": 0.2}, samplemethod=method)
fig, _ = dist.plot(title="dist=geom, distkwargs={'p': 0.2}")
fig.set_size_inches(6.0, 2.5)
plt.savefig("docs/images/geom.png", dpi=100)

dist = InVar("Poisson", ndraws=n, dist=poisson, distkwargs={"mu": 2}, samplemethod=method)
