...
def plot(
    ...
    if vary is None and varz is None:
        if varx.maxdim not in (0, 1, 2):
            ...
            if len(varx_split) == 3:
                ...
                raise ValueError(
                    "Can only split a single variable into 3 vectors:"
    ...
    # Split one of two vars that is a 2D vector
        ...
        if varx.maxdim == 2 and vary.maxdim == 0 and isinstance(varx, monaco.mc_var.OutVar):
            ...
            if len(varx_split) > 2:
                raise ValueError(
                    "Can only split one of two variables into 2 vectors:"
                    + f"{origname}: {len(varx_split)}"
                )
...
def plot_hist(
    ...
    if orientation == PlotOrientation.VERTICAL:
        ...
        for i in highlight_cases_list:
            plt.plot(
                [xlim[0], xlim[0] + (xlim[1] - xlim[0]) * 0.20],
                [points[i], points[i]],
                linestyle="-",
                linewidth=1,
                color="C1",
        ...
        for varstat in var.varstats:
            ...
            if length(nums) == 1:  # Single Statistic
                ...
                plt.plot(xlim, [nums[1], nums[1]], linestyle="-", color="C0", alpha=1, **plotkwargs)
            if varstat.confidence_interval_high_nums is not None:
                ax.fill_between(
                    xlim,
                    [varstat.confidence_interval_low_nums, varstat.confidence_interval_high_nums],
...
def plot_2d_scatter(
    ...
    if varz is not None:
        ...
        zi = griddata(
            (varx_points, vary_points), varz_points, (xi[None, :], yi[:, None]), method="linear"
        )
    ...
    if reg_cases:
        plt.scatter(
            slice_by_index(varx_points, reg_cases),
    ...
    if highlight_cases_list:
        plt.scatter(
            slice_by_index(varx_points, highlight_cases_list),
    ...
    if rug_plot:
        ...
        plot_rug_marks(
            ax, orientation=PlotOrientation.VERTICAL, nums=slice_by_index(varx_points, all_cases)
        )
        plot_rug_marks(
            ax, orientation=PlotOrientation.HORIZONTAL, nums=slice_by_index(vary_points, all_cases)
        )
...
def plot_2d_line(
    ...
    for i in reg_cases:
        plt.plot(
            varx_points[i], vary_points[i], linestyle="-", color="black", alpha=0.2, **plotkwargs
        )
    for i in highlight_cases_list:
...
def plot_rug_marks(ax: Axes, orientation: PlotOrientation, nums: Iterable[float]) -> None:
    """
    ...
    if orientation == PlotOrientation.VERTICAL:
        for num in nums:
            plt.plot(
                [num, num],
                [ylim[0], ylim[0] + 0.02 * (ylim[1] - ylim[0])],
                color="black",
                linewidth=0.5,
                alpha=0.5,
            )
        ...
        for num in nums:
            plt.plot(
                [xlim[0], xlim[0] + 0.02 * (xlim[1] - xlim[0])],
                [num, num],
                color="black",
                linewidth=0.5,
                alpha=0.5,
...
def plot_2d_cov_ellipse(
    ...
    ellipse = Ellipse(
        center,
        2 * ellipse_axis_radii[0],
        2 * ellipse_axis_radii[1],
        angle=angle,
        fill=False,
        edgecolor="k",
    )
...
def manage_invar_space(
    invar_space: InVarSpace | Iterable[InVarSpace],
    nvars: int,
) -> list[InVarSpace]:
    """
