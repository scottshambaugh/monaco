    ...
    if samplemethod == SampleMethod.RANDOM:
        ...
        try:
            rootResults = root_scalar(
                f, method="brentq", bracket=[2**8, 2**31 - 1], xtol=0.1, maxiter=int(1e3)
            )
...
def integration_args_check(
    ...
    if samplemethod not in (SampleMethod.RANDOM, SampleMethod.SOBOL):
        raise ValueError(
            f"{samplemethod=} must be either " + f"{SampleMethod.RANDOM} or {SampleMethod.SOBOL}"
        )
