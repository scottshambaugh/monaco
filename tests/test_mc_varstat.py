...
def test_invarstat(stattype, statkwargs, vals, invar):
    invarstat = VarStat(invar, stattype=stattype, statkwargs=statkwargs)
    assert np.allclose(invarstat.vals, vals)


def test_varstat_setName(invar):
    bound = StatBound.ONESIDED
    invarstat = VarStat(invar, stattype=VarStatType.ORDERSTATTI,
...
def test_outvarstat_2d():
    ...
    outvarstat1 = VarStat(outvar, stattype=VarStatType.ORDERSTATTI,
                              statkwargs={'p': 0.6, 'c': 0.50, 'bound': StatBound.ALL})
    outvarstat2 = VarStat(outvar, stattype=VarStatType.MIN)
...
def test_outvarstat_2d_irregular():
    ...
    outvarstat = VarStat(outvar, stattype=VarStatType.MIN)
