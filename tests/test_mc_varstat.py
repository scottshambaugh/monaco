...
@pytest.mark.parametrize(
    "stat, statkwargs, vals, cilow, cihigh",
    [
        (
            VarStatType.ORDERSTATTI,
            {"p": sig2pct(3, bound), "c": 0.50, "bound": bound},
            3.0069887,
            [],
            [],
        ),
...
def test_varstat_nummap():
    ...
    # 1-D variable
    valmap = {"a": 1, "b": 2}
    outvar = OutVar("test", [["a", "b", "a"], ["b", "a", "b"], ["a", "b", "a"]], valmap=valmap)
...
def test_outvarstat_2d_irregular(v):
    ...
    outvarstat2 = VarStat(
        outvar,
        stat=VarStatType.PERCENTILE,
        statkwargs={"p": [0.25, 0.75]},
        bootstrap=True,
        bootstrap_k=10,
    ...
    assert np.allclose(
        outvarstat2.vals,
        [[-1.5, 0], [-0.75, 0], [-0.5, 2.5], [-0.75, 3.75], [-1.0, 5.0], [-1.25, 6.25]],
    )  # noqa: E501
    assert np.allclose(
        outvarstat2.confidence_interval_low_nums,
        [[-3, -1.875], [-1.5, -0.9375], [-4.25, 1], [-6.375, 1.5], [-8.5, 2], [-10.625, 2.5]],
    )  # noqa: E501
    assert np.allclose(
        outvarstat2.confidence_interval_high_nums,
        [[0.5, 0.875], [0.25, 0.4375], [1, 6.25], [1.5, 9.375], [2, 12.5], [2.5, 15.625]],
    )  # noqa: E501
