...
from monaco.mc_enums import SampleMethod, StatBound, VarStat
...
def mcinvar():
    ...
    seed = 74494861
    return MCInVar('norm', ndraws=100000,
...
@pytest.mark.parametrize("stattype, statkwargs, vals", [
    (VarStat.ORDERSTATTI, {'p': sig2pct( 3, bound), 'c': 0.50, 'bound': bound}        ,  3.0060487),
    (VarStat.ORDERSTATP , {'p': sig2pct( 3, bound), 'c': 0.50, 'bound': StatBound.ALL}, [2.9965785, 3.0069887, 3.0209262]),  # noqa: E501
    (VarStat.GAUSSIANP  , {'p': sig2pct(-3, bound), 'bound': bound}                   , -3.0021598),
...
def test_mcvarstat_setName(mcinvar):
    bound = StatBound.ONESIDED
    mcinvarstat = MCVarStat(mcinvar, stattype=VarStat.ORDERSTATTI,
...
def test_mcoutvarstat_2d():
    ...
    mcoutvarstat1 = MCVarStat(outvar, stattype=VarStat.ORDERSTATTI,
                              statkwargs={'p': 0.6, 'c': 0.50, 'bound': StatBound.ALL})
    mcoutvarstat2 = MCVarStat(outvar, stattype=VarStat.MIN)
    assert(np.allclose(mcoutvarstat1.vals, [[-4, -2, 4], [-2, -1, 2], [-4, -2, 4],
                                            [-6, -3, 6], [-8, -4, 8], [-10, -5, 10]]))
