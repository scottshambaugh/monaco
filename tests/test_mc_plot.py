...
def plot_testing(show=False):
    ...
    ndraws = 1000
    invars["randint"] = InVar(
        name="randint",
        ndraws=ndraws,
        dist=randint,
        distkwargs={"low": 1, "high": 6},
        nummap=nummap,
    ...
    invars["norm"].addVarStat(
        stat="orderstatTI", statkwargs={"p": 0.75, "c": 0.50, "bound": "2-sided"}
    )
    invars["norm"].addVarStat(stat="orderstatP", statkwargs={"p": 0.5, "c": 0.9999, "bound": "all"})
    invars["norm2"] = InVar(
        name="norm2",
        ndraws=ndraws,
        dist=norm,
    ...
    plot_cov_corr(
        np.array([[2, 0.1111, np.nan], [-0.19, -1, np.nan], [np.nan, np.nan, np.nan]]),
        ["Test1", "Test2", "Test3"],
    )

    invars["randint2"] = InVar(
        name="randint2",
        ndraws=1000,
        dist=randint,
        distkwargs={"low": 0, "high": 2},
        samplemethod=SampleMethod.RANDOM,
        seed=invarseeds[3],
    )
    plot_integration_convergence(invars["randint2"], volume=1, dimension=1, refval=0.5, conf=0.95)
