...
def test_integration_error():
    assert integration_error(
        [1, 0, 2], dimension=1, conf=0.95, samplemethod=SampleMethod.RANDOM, runningerror=False
    ) == pytest.approx(0.4619679)
    assert integration_error(
        [1, 0, 2], dimension=1, conf=0.95, samplemethod=SampleMethod.RANDOM, runningerror=True
    ) == pytest.approx([0.8001519, 0.5657928, 0.4619679])
    assert integration_error(
        [1, 0, 2], dimension=1, conf=0.95, samplemethod=SampleMethod.SOBOL, runningerror=False
    ) == pytest.approx(0.4619679)
    assert integration_error(
        [1, 0, 2], dimension=1, conf=0.95, samplemethod=SampleMethod.SOBOL, runningerror=True
    ) == pytest.approx([0.4803176, 0.4803176, 0.4619679])
    assert integration_error(
        [
            1,
        ],
        ...
        runningerror=True,
    ) == pytest.approx(pct2sig(0.95))


def test_integration_n_from_err():
    assert (
        integration_n_from_err(
            error=0.01, dimension=1, volume=1, stdev=1, conf=0.95, samplemethod=SampleMethod.RANDOM
        )
        == 38415
    )
    assert (
        integration_n_from_err(
            error=0.01, dimension=1, volume=1, stdev=1, conf=0.95, samplemethod=SampleMethod.SOBOL
        )
        == 1424
    )
...
@pytest.mark.parametrize(
    "error,dimension,volume,stdev,conf,samplemethod",
    [
...
def plot_testing(show=False):
    ...
    err1 = integration_error(
        x1, dimension=1, volume=1, conf=conf, samplemethod=SampleMethod.RANDOM, runningerror=True
    )
    err2 = integration_error(
        x2, dimension=1, volume=1, conf=conf, samplemethod=SampleMethod.SOBOL, runningerror=True
    )
    alpha = 0.85
    plt.figure()
