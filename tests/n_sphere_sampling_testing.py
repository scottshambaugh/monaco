...
distancerandom = [np.sqrt(np.sum([samplepointsrandom[i][j]**2 for i in range(d)])) for j in range(n)]
distancesobol  = [np.sqrt(np.sum([ samplepointssobol[i][j]**2 for i in range(d)])) for j in range(n)]
