...
def test_mcinval():
    ...
    inval = MCInVal(name='test', ncase=1, pct=0.5, num=0, dist=norm, ismean=True)
    assert inval.val == 0
    
    inval = MCInVal(name='test', ncase=1, pct=0.5, num=0, nummap={0:'a'}, dist=norm, ismean=True)
    assert inval.val == 'a'
...
@pytest.fixture
def mcoutval_3_by_2():
    return MCOutVal(name='test', ncase=1, val=[[0,0],[0,0],[0,0]], ismean=True)
...
def test_mcoutval_valmap_str():
    outval = MCOutVal(name='test', ncase=1, val=[['a','a'],['b','b'],['a','b']], ismean=True)
...
def test_mcoutval_dataframe():
    ...
    mcoutval1 = MCOutVal(name='test1', ncase=1, val=df['vals1'], ismean=True)
    assert all(mcoutval1.num == [0, 1, 2])
    
    mcoutval2 = MCOutVal(name='test2', ncase=1, val=df.index, ismean=True)
...
def inline_testing():
    from scipy.stats import norm
    a = MCInVal(name='TestA', ncase=1, pct=0.5, num=0, dist=norm, ismean=True)
    print(a.val) # expected: 0
    b = MCInVal(name='TestB', ncase=1, pct=0.5, num=0, nummap={0:'a'}, dist=norm, ismean=True)
    print(b.val) # expected: a
    c = MCOutVal(name='TestC', ncase=1, val=[[0,0],[0,0],[0,0]], ismean=True)
    print(c.size) # expected: (3, 2)
    print(c.val) # expected: [[0, 0], [0, 0], [0, 0]]
    csplit = c.split()
    print(csplit['TestC [0]'].val) # expected: [0, 0]
    d = MCOutVal(name='TestD', ncase=1, val=[['a','a'],['b','b'],['a','b']], ismean=True)
    ...
    dates = pd.date_range(start='2020-01-01', periods=nvals, freq='YS')
    df = pd.DataFrame({'vals1': range(nvals), 'vals2': range(nvals)}, index = dates)
    g = MCOutVal(name='TestG', ncase=1, val=df['vals1'], ismean=True)
    print(g.num) # expected: [0 1 2]
    h = MCOutVal(name='TestH', ncase=1, val=df.index, ismean=True)
