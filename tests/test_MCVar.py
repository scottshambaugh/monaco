...
def mcinvar_norm_random():
    ...
    return MCInVar('norm', ndraws=1000, dist=norm, distkwargs={'loc':10, 'scale':4}, seed=invarseeds[1], samplemethod=SampleMethod.RANDOM, firstcaseismean=False)

@pytest.fixture
def mcinvar_custom_dist():
    return MCInVar('custom', ndraws=1000, dist=custom_dist, distkwargs=dict(), ninvar=1, samplemethod=SampleMethod.RANDOM, seed=invarseeds[2], firstcaseismean=False)

def test_mcinvar_norm_sobol_warning():
    from scipy.stats import norm
    with pytest.warns(UserWarning, match='Infinite value'):
        MCInVar('norm', ndraws=1000, dist=norm, distkwargs={'loc':10, 'scale':4}, seed=invarseeds[1], samplemethod=SampleMethod.SOBOL, ninvar=1, firstcaseismean=False)
...
def test_mcinvar_nummap():
    invar = MCInVar('map', ndraws=10, dist=custom_dist, distkwargs=dict(), ninvar=1, nummap={1:'a',5:'e',6:'f'}, samplemethod=SampleMethod.RANDOM, seed=invarseeds[3], firstcaseismean=False)
    assert invar.vals == ['f', 'e', 'f', 'f', 'a', 'e', 'e', 'a', 'e', 'e']



def test_mcoutvar():
    outvar = MCOutVar('test', [1, 0, 2, 2], firstcaseismean=True)
    assert outvar.getVal(1).val == 0
    assert outvar.stats().mean == pytest.approx(1.25)
    assert outvar.getMean().val == 1

def test_mcoutvar_extractValMap():
    outvar = MCOutVar('test', ['a', 'b', 'c', 'b'], firstcaseismean=True)
...
def inline_testing():
    ...
    mcinvars = dict()
    mcinvars['randint'] = MCInVar('randint', ndraws=1000, dist=randint, distkwargs={'low':1, 'high':5}, seed=invarseeds[0], samplemethod=SampleMethod.RANDOM, firstcaseismean=False)
    ...
    mcinvars['norm2'] = MCInVar('norm', ndraws=1000, dist=norm, distkwargs={'loc':10, 'scale':4}, seed=invarseeds[1], samplemethod=SampleMethod.RANDOM, firstcaseismean=False) 
    ...
    pk = np.ones(len(xk))/len(xk)
    custom_dist = rv_discrete(name='custom', values=(xk, pk))
    mcinvars['custom'] = MCInVar('custom', ndraws=1000, dist=custom_dist, distkwargs=dict(), ninvar=1, samplemethod=SampleMethod.RANDOM, seed=invarseeds[2], firstcaseismean=False)
    ...
    mcinvars['map'] = MCInVar('map', ndraws=10, dist=custom_dist, distkwargs=dict(), ninvar=1, nummap={1:'a',5:'e',6:'f'}, samplemethod=SampleMethod.RANDOM, seed=invarseeds[3], firstcaseismean=False)
    ...
    mcoutvars = dict()
    mcoutvars['test'] = MCOutVar('test', [1, 0, 2, 2], firstcaseismean=True)
    ...
    print(mcoutvars['test'].getMean().val) # expected: 1
    mcoutvars['test2'] = MCOutVar('test2', ['a', 'b', 'c', 'b'], firstcaseismean=True)
