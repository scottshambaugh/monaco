...
fcns = {
    "preprocess": pandemic_example_preprocess,
...
def pandemic_example_monte_carlo_sim():
    ...
    sim.addInVar(
        name="Probability of Infection", dist=uniform, distkwargs={"loc": 0.28, "scale": 0.04}
    )

    sim.runSim()

    sim.outvars["Proportion Infected"].addVarStat(
        stat="orderstatTI", statkwargs={"p": p, "c": c, "bound": bound}
    )
    sim.outvars["Superspreader Degree"].addVarStat(
        stat="orderstatTI", statkwargs={"p": 0.5, "c": 0.5, "bound": "all"}
    )
