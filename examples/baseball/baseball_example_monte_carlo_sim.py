...
def baseball_example_monte_carlo_sim():
    ...
    sim.addInVar(
        name="Z Init [m]", dist=uniform, distkwargs={"loc": 18.29 * in2m, "scale": 25.79 * in2m}
    )
    sim.addInVar(
        name="Speed Init [m/s]", dist=norm, distkwargs={"loc": 90 * mph2mps, "scale": 5 * mph2mps}
    )
