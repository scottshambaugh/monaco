...
fcns = {
    "preprocess": retirement_example_preprocess,
...
def retirement_example_monte_carlo_sim():
    ...
    for i in range(nyears):
        sim.addInVar(
            name=f"Year {i} Returns",
            dist=norm,
            distkwargs={"loc": (sp500_mean - inflation), "scale": sp500_stdev},
        )

    sim.addInVar(
        name="Beginning Balance", dist=uniform, distkwargs={"loc": 1000000, "scale": 100000}
    )
