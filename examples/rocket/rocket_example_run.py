...
def rocket_example_run(sequence, massprops, propulsion, aero, launchsite):
    ...
    # flightstage is prelaunch, ignition, poweredflight, coastflight, parachute, or landed
    flightstage = ['prelaunch']
    ...
    # Note: x = easting, y = northing, z = alt above sea level (vertical)
    pos = np.array([[0, 0, 0]])  # position [x,y,z] [m]
    vel = np.array([[0, 0, 0]])  # velocity [vx,vy,vz] [m/s]
    acc = np.array([[0, 0, 0]])  # acceleration [ax,ay,az] [m/s^2]

    windvel = aero['windspd']*np.array([-np.sin(d2r*aero['windazi']),
                                        -np.cos(d2r*aero['windazi']),
                                        0])
    thrustvec = np.array([np.sin(d2r*launchsite['launchangle']),
                          np.sin(d2r*launchsite['launchangle']),
                          np.cos(d2r*launchsite['launchangle'])]) \
                * np.array([np.sin(d2r*launchsite['launchazi']),
                            np.cos(d2r*launchsite['launchazi']),
                            1])
    ...
    while t[i] <= tmax:
        ...
        if flightstage[i] == 'prelaunch':
            ...
            if vel[i-2][2] >= 0 and vel[i-1][2] <= 0:
                tapogee = t[i-1]
            if tapogee is not None \
               and t[i] >= (tapogee + sequence['parachute_delay'])\
               and not sequence['parachute_fails']:
                flightstage[i] = 'parachute'

        # Look up aero data based on if parachute is deployed
        if flightstage[i] == 'parachute':
            area = np.array([aero['area_ax_parachute'],
                             aero['area_lat_parachute'],
                             aero['area_lat_parachute']])
            cd = np.array([aero['cd_ax_parachute'],
                           aero['cd_lat_parachute'],
                           aero['cd_lat_parachute']])
        else:
            area = np.array([aero['area_ax_flight'],
                             aero['area_lat_flight'],
                             aero['area_lat_flight']])
            cd = np.array([aero['cd_ax_flight'],
                           aero['cd_lat_flight'],
                           aero['cd_lat_flight']])

        # Look up thrust and mass flow data
        mdot = np.interp(t[i]-sequence['ignition'], propulsion['prop_time'],
                         propulsion['prop_massflow'])
        m = np.append(m, m[i-1] + mdot*dt)
        T = np.append(T, np.interp(t[i]-sequence['ignition'],
                      propulsion['prop_time'], propulsion['prop_thrust']))
...
if __name__ == '__main__':
    sequence = {
        ...
        'parachute_fails' : True, # whether the parachute fails to deploy (boolean)
        }
    ...
    (t, m, flightstage, T, pos, vel, acc) \
        = rocket_example_run(sequence, massprops, propulsion, aero, launchsite)
