...
def rocket_example_monte_carlo_sim():
    sim = MCSim(name='Rocket', ndraws=ndraws, fcns=fcns, firstcaseismedian=True,
    ...
    mc_plot(sim.mcoutvars['Easting [m]'], sim.mcoutvars['Northing [m]'],
            sim.mcoutvars['Altitude [m]'], title='Model Rocket Trajectory', highlight_cases=0)
    # import matplotlib.pyplot as plt
    # plt.savefig('rocket_trajectory.png')
    mc_multi_plot([sim.mcinvars['Wind Speed [m/s]'], sim.mcoutvars['Landing Dist [m]']],
