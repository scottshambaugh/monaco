...
def mcinvar_norm_random():
    ...
    return MCInVar('norm', ndraws=1000, dist=norm, distkwargs={'loc':10, 'scale':4}, seed=invarseeds[1], samplemethod='random', firstcaseisnom=False)

@pytest.fixture
def mcinvar_custom_dist():
    return MCInVar('custom', ndraws=1000, dist=custom_dist, distkwargs=dict(), ninvar=1, samplemethod='random', seed=invarseeds[2], firstcaseisnom=False)

def test_mcinvar_norm_sobol_warning():
    from scipy.stats import norm
    with pytest.warns(UserWarning, match='Infinite value'):
        MCInVar('norm', ndraws=1000, dist=norm, distkwargs={'loc':10, 'scale':4}, seed=invarseeds[1], samplemethod='sobol', ninvar=1, firstcaseisnom=False)
...
def test_mcinvar_nummap():
    invar = MCInVar('map', ndraws=10, dist=custom_dist, distkwargs=dict(), ninvar=1, nummap={1:'a',5:'e',6:'f'}, samplemethod='random', seed=invarseeds[3], firstcaseisnom=False)
...
if __name__ == '__main__':
    ...
    mcinvars = dict()
    mcinvars['randint'] = MCInVar('randint', ndraws=1000, dist=randint, distkwargs={'low':1, 'high':5}, seed=invarseeds[0], samplemethod='random', firstcaseisnom=False)
    ...
    mcinvars['norm2'] = MCInVar('norm', ndraws=1000, dist=norm, distkwargs={'loc':10, 'scale':4}, seed=invarseeds[1], samplemethod='random', firstcaseisnom=False) 
    ...
    pk = np.ones(len(xk))/len(xk)
    custom_dist = rv_discrete(name='custom', values=(xk, pk))
    mcinvars['custom'] = MCInVar('custom', ndraws=1000, dist=custom_dist, distkwargs=dict(), ninvar=1, samplemethod='random', seed=invarseeds[2], firstcaseisnom=False)
    ...
    mcinvars['map'] = MCInVar('map', ndraws=10, dist=custom_dist, distkwargs=dict(), ninvar=1, nummap={1:'a',5:'e',6:'f'}, samplemethod='random', seed=invarseeds[3], firstcaseisnom=False)
